...
sequenceDiagram
    ...
    Note right of Wallet: wallet must have proper key
    Note right of ONCHAINID: checks if wallet has key
    Note right of ONCHAINID: performs action if wallet has key
    Note right of ONCHAINID: sends success/failure notification
    Note right of Wallet: receives success/failure notification
```
...
- `scheme`: The scheme with which this claim SHOULD be verified or how it should be processed. Its a `uint256` for 
  different schemes. Those are used on ONCHAINID to specify the key types e.g. 1 = ECDSA, 2 = RSA, etc. The 
  verification of the signature by key type should be implemented in the `isClaimValid` function on the `ClaimIssuer`
  contract
...
Once the claim is verified, the function stores the data in the claim and pushes the claimId to the claimsByTopic array. This array keeps track of all the claims associated with a particular topic. 
